...
class TimeStep {
  ...
  public:
     ...
     * @return T  start time
     */
    T start_time( void ) const { return m_start_time; }
     ...
     * @return T end time
     */
    T end_time( void ) const { return m_end_time; }
     ...
     */
    T increment_time( void ) const { return m_end_time - m_start_time; }
...
class ListOfTimeStep {
  private:
    ...
    template < typename I >
    void addTimeStepping( const T end_time, const I n_step, const size_t level ) {
        T start_time = T( 0 );
        ...
        for ( I i = 0; i < n_step; i++ ) {
            if ( ( start_time + ( i + 1 ) * delta_t ) <= user_end_time ) {
                const TimeStep< T > step( start_time + i * delta_t,
                                          start_time + ( i + 1 ) * delta_t, level );
  ...
  public:
    ...
    ListOfTimeStep( const T end_time, const size_t n_step )
        : n_time_step_comp( 0 ), user_end_time( end_time ) {
        this->addTimeStepping( end_time, n_step, 0 );
    }

    template < typename I >
    ListOfTimeStep( const std::vector< std::pair< T, I > > list_of_time_step )
        : n_time_step_comp( 0 ) {
    ...
    template < typename I >
    ListOfTimeStep( const std::vector< std::pair< T, I > > list_of_time_step, const T final_time )
        : n_time_step_comp( 0 ) {
     ...
     */
    bool empty( void ) const { return list_steps.empty(); }
     ...
     */
    size_t numberOfTimeStep( void ) const { return list_steps.size() + n_time_step_comp; }
     ...
     * @return size_t number of time step
     */
    size_t numberOfRemainingTimeStep( void ) const { return list_steps.size(); }
     ...
     */
    size_t numberOfTimeStepRealized( void ) const { return n_time_step_comp; }
     ...
     */
    size_t indexOfCurrentTimeStep( void ) const { return n_time_step_comp + 1; }
     ...
     */
    TimeStep< T > getCurrentTimeStep( void ) const { return list_steps.front(); }

    /**
     * @brief Remove the current time step
     *
     */
    void removeCurrentTimeStep( void ) {
        list_steps.pop_front();
        n_time_step_comp++;
    }

    /**
     * @brief Split the Current time step in two equal sub-time step. This allows to refine the time
     * step
     *
     */
    void splitCurrentTimeStep( void ) {
     ...
     *
     */
    void printCurrentTimeStep( void ) const {
        ...
        std::cout << "************************ Time : " << current_time
                  << " sec (step: " << this->indexOfCurrentTimeStep() << "/"
