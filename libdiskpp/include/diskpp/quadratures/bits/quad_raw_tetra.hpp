...
static const double arbq_to_ref_b[3] = {
    -1.000000000000000,
    -0.577350269189626,
    -0.408248290463863
};
...
arbq(size_t degree, const point<T,3>& p0, const point<T,3>& p1,
    const point<T,3>& p2, const point<T,3>& p3)
{
    int rule = degree;
    if (rule == 0)
        rule = 1;
...
template<typename T>
std::vector<quadrature_point<T,3>>
grundmann_moeller(size_t degree, const point<T,3>& p0, const point<T,3>& p1,
    const point<T,3>& p2, const point<T,3>& p3)
{
