...
Data::Data(const size_t index, const std::vector<double>& data) : m_index(index), m_data(data) {}

size_t
Data::getNbComposante() const
{
   return m_index;
}
...
void
Data::changeData(const size_t indice, const double value)
{
   m_data.at(indice) = value;
}
void
Data::changeData(const std::vector<double>& data)
{
   m_data.clear();
...
void
Data::changeIndex(const size_t index)
{
   m_index = index;
}
...
SubData::SubData(const Data& data, const Node& node) : SubData(data.getData(), node) {}

Node
SubData::getNode() const
{
   return m_node;
}

void
SubData::changeCoordinates(const std::array<double, 3>& coor)
{
   m_node.changeCoordinates(coor);
}

void
SubData::changeIndex(const size_t index)
{
   m_index = index;
...
GenericData::GenericData(const size_t             nbcompo,
                         const double             time_value,
                         const std::string        title,
                         const std::vector<Data>& datas) :
  m_composante(nbcompo),
  m_time_value(time_value), m_title(title), m_datas(datas)
{}

std::vector<Data>
GenericData::getData() const
{
   return m_datas;
}

double
GenericData::getTime() const
{
   return m_time_value;
}

std::string
GenericData::getTitle() const
{
   return m_title;
}

size_t
GenericData::getNbComposante() const
{
   return m_composante;
}

void
GenericData::changeNbComposante(const size_t nbcompo)
{
   m_composante = nbcompo;
}

void
GenericData::changeTime(const double time)
{
   m_time_value = time;
}

void
GenericData::changeTitle(const std::string title)
{
   m_title = title;
}

void
GenericData::addData(const Data& data)
{
   m_datas.push_back(data);
}

void
GenericData::addData(const size_t index, const std::vector<double>& values)
{
...
NodeData::NodeData(const size_t                nbcompo,
                   const double                time_value,
                   const std::string           title,
                   const std::vector<Data>&    datas,
                   const std::vector<SubData>& subdatas) :
  GenericData(nbcompo, time_value, title, datas),
  m_subdatas(subdatas)
{}

void
NodeData::addSubData(const Data& subdata, const Node& node)
{
...
void
NodeData::addSubData(const std::vector<double>& subdata, const Node& node)
{
   SubData tmp(subdata, node);
   m_subdatas.push_back(tmp);
}
...
void
NodeData::writeNodeData(const std::string name_mesh) const
{
...
void
NodeData::saveNodeData(const std::string name_mesh, const Gmesh& mesh)
{
   Gmesh meshtmp(mesh);
...
void
ElementData::writeElementData(const std::string name_mesh) const
{
   std::ofstream mesh_file;
   mesh_file.open(name_mesh, std::ofstream::out | std::ofstream::app);
   if (!mesh_file.is_open()) {
...
void
ElementData::saveElementData(const std::string name_mesh, const Gmesh& mesh) const
{
...
void
ElementNodeData::writeElementNodeData(const std::string name_mesh) const
{
   std::ofstream mesh_file;
   mesh_file.open(name_mesh, std::ofstream::out | std::ofstream::app);
   if (!mesh_file.is_open()) {
...
void
ElementNodeData::saveElementNodeData(const std::string name_mesh, const Gmesh& mesh) const
{
   mesh.writeGmesh(name_mesh, 2);
