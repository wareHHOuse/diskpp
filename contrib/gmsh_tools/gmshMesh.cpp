...
Gmesh::Gmesh() :
  _dim_topology(0), _nodes(), _vertices(), _edges(), _triangles(), _quadrangles(), _tetrahedra(),
  _hexahedra(), _prisms(), _pyramids(), _number_of_elements(0)
{}

Gmesh::Gmesh(size_t dim) :
  _dim_topology(dim), _nodes(), _vertices(), _edges(), _triangles(), _quadrangles(), _tetrahedra(),
  _hexahedra(), _prisms(), _pyramids(), _number_of_elements(0)
{}

Gmesh::Gmesh(const size_t                dim,
             const std::vector<Node>&    nodes,
             const std::vector<Vertice>& vertices,
             const std::vector<Edge>&    edges) :
  _dim_topology(dim),
  _nodes(nodes), _vertices(vertices), _edges(edges), _triangles(), _quadrangles(), _tetrahedra(),
  _hexahedra(), _prisms(), _pyramids(), _number_of_elements(vertices.size() + edges.size())
{}
...
Gmesh::Gmesh(const size_t                   dim,
  ...
  _dim_topology(dim),
  _nodes(nodes), _vertices(vertices), _edges(edges), _triangles(triangles),
  _quadrangles(quadrangles), _tetrahedra(), _hexahedra(), _prisms(), _pyramids(),
...
Gmesh::Gmesh(const size_t                    dim,
  ...
  _dim_topology(dim),
  _nodes(nodes), _vertices(vertices), _edges(edges), _triangles(triangles),
  _quadrangles(quadrangles), _tetrahedra(tetrahedra), _hexahedra(hexahedra), _prisms(), _pyramids(),
  _number_of_elements(vertices.size() + edges.size() + triangles.size() + quadrangles.size() +
...
Gmesh::Gmesh(const size_t                    dim,
  ...
  _dim_topology(dim),
  _nodes(nodes), _vertices(vertices), _edges(edges), _triangles(triangles),
  _quadrangles(quadrangles), _tetrahedra(tetrahedra), _hexahedra(hexahedra), _prisms(prisms),
  _pyramids(pyramids),
  _number_of_elements(vertices.size() + edges.size() + triangles.size() + quadrangles.size() +
...
Gmesh::Gmesh(const size_t                    dim,
             ...
             const std::vector<Tetrahedron>& tetrahedra) :
  _dim_topology(dim),
  _nodes(nodes), _vertices(vertices), _edges(edges), _triangles(triangles), _quadrangles(),
  _tetrahedra(tetrahedra), _hexahedra(), _prisms(), _pyramids(),
  _number_of_elements(vertices.size() + edges.size() + triangles.size() + tetrahedra.size())
{}
...
Gmesh::Gmesh(const size_t                   dim,
             ...
             const std::vector<Hexahedron>& hexahedra) :
  _dim_topology(dim),
  _nodes(nodes), _vertices(vertices), _edges(edges), _triangles(), _quadrangles(quadrangles),
  _tetrahedra(), _hexahedra(hexahedra), _prisms(), _pyramids(),
  _number_of_elements(vertices.size() + edges.size() + quadrangles.size() + hexahedra.size())
{}

size_t
Gmesh::getNumberofNodes() const
{
   return _nodes.size();
}

size_t
Gmesh::getNumberofElements() const
{
...
std::vector<Node>
Gmesh::getNodes() const
{
   return _nodes;
}

std::vector<Vertice>
Gmesh::getVertices() const
{
   return _vertices;
}

std::vector<Edge>
Gmesh::getEdges() const
{
   return _edges;
}

std::vector<Triangle>
Gmesh::getTriangles() const
{
   return _triangles;
}

std::vector<Quadrangle>
Gmesh::getQuadrangles() const
{
   return _quadrangles;
}

std::vector<Tetrahedron>
Gmesh::getTetrahedra() const
{
   return _tetrahedra;
}

std::vector<Hexahedron>
Gmesh::getHexahedra() const
{
   return _hexahedra;
}

std::vector<Prism>
Gmesh::getPrisms() const
{
   return _prisms;
}

std::vector<Pyramid>
Gmesh::getPyramids() const
{
   return _pyramids;
}

Node
Gmesh::getNode(const size_t index) const
{
   return _nodes.at(index);
}

Vertice
Gmesh::getVertice(const size_t index) const
{
   return _vertices.at(index);
}

Edge
Gmesh::getEdge(const size_t index) const
{
   return _edges.at(index);
}

Triangle
Gmesh::getTriangle(const size_t index) const
{
   return _triangles.at(index);
}

Quadrangle
Gmesh::getQuadrangle(const size_t index) const
{
   return _quadrangles.at(index);
}

Hexahedron
Gmesh::getHexahedron(const size_t index) const
{
   return _hexahedra.at(index);
}

Tetrahedron
Gmesh::getTetrahedron(const size_t index) const
{
   return _tetrahedra.at(index);
}

Prism
Gmesh::getPrism(const size_t index) const
{
   return _prisms.at(index);
}

Pyramid
Gmesh::getPyramid(const size_t index) const
{
   return _pyramids.at(index);
}

void
Gmesh::readGmesh_MEDITformat(const std::string name_mesh)
{
   ...
   while (!mesh_file.eof()) {
      ...
      if (file_line.find("Hexahedra") != std::string::npos) {
         ...
         for (size_t i = 0; i < nb_hexa; i++) {
            mesh_file >> hexa[0] >> hexa[1] >> hexa[2] >> hexa[3] >> hexa[4] >> hexa[5] >>
              hexa[6] >> hexa[7] >> hexa[8] >> ref;
...
void
Gmesh::readGmesh_MSHformat(const std::string name_mesh)
{
   std::ifstream mesh_file(name_mesh.data());
   if (!mesh_file.is_open()) {
      std::cout << "Unable to open file " << name_mesh << std::endl;
...
void
Gmesh::readGmesh(const std::string name_mesh)
{
...
void
Gmesh::writeGmesh_MEDITformat(const std::string name_mesh) const
{
   ...
   for (size_t i = 0; i < _nodes.size(); i++) {
      Node node(_nodes[i]);
      mesh_file << (node.getCoordinate())[0] << " " << (node.getCoordinate())[1] << " "
                << (node.getCoordinate())[2] << " " << node.getRef() << std::endl;
   }
   ...
   for (size_t i = 0; i < _edges.size(); i++) {
      Edge edge(_edges[i]);
      mesh_file << (edge.getNodes())[0] << " " << (edge.getNodes())[1] << " "
                << edge.getPhysicalEntities() << std::endl;
   }
   ...
   for (size_t i = 0; i < _triangles.size(); i++) {
      ...
      mesh_file << (tri.getNodes())[0] << " " << (tri.getNodes())[1] << " " << (tri.getNodes())[2]
                << " " << tri.getPhysicalEntities() << std::endl;
   }
   ...
   for (size_t i = 0; i < _quadrangles.size(); i++) {
      ...
      mesh_file << (quad.getNodes())[0] << " " << (quad.getNodes())[1] << " "
                << (quad.getNodes())[2] << " " << (quad.getNodes())[3] << " "
                << quad.getPhysicalEntities() << std::endl;
   }
   ...
   for (size_t i = 0; i < _tetrahedra.size(); i++) {
      ...
      mesh_file << (tetra.getNodes())[0] << " " << (tetra.getNodes())[1] << " "
                << (tetra.getNodes())[2] << " " << (tetra.getNodes())[3] << " "
                << tetra.getPhysicalEntities() << std::endl;
   }
   ...
   for (size_t i = 0; i < _quadrangles.size(); i++) {
      ...
      mesh_file << (hexa.getNodes())[0] << " " << (hexa.getNodes())[1] << " "
                << (hexa.getNodes())[2] << " " << (hexa.getNodes())[3] << " "
                << (hexa.getNodes())[4] << " " << (hexa.getNodes())[5] << " "
                << (hexa.getNodes())[6] << " " << (hexa.getNodes())[7] << " "
                << hexa.getPhysicalEntities() << std::endl;
   }
...
void
Gmesh::writeGmesh_MSHformat(const std::string name_mesh) const
{
   std::ofstream mesh_file(name_mesh);
   if (!mesh_file.is_open()) {
      std::cout << "Unable to open file " << name_mesh << std::endl;
   ...
   for (size_t i = 0; i < _nodes.size(); i++) {
      ...
      mesh_file << node.getIndex() << " " << (node.getCoordinate())[0] << " "
                << (node.getCoordinate())[1] << " " << (node.getCoordinate())[2] << std::endl;
   }
   ...
   for (size_t i = 0; i < _vertices.size(); i++) {
      ...
      mesh_file << offset_elements << " " << 15 << " " << 2 //number of tag
                << " " << vert.getPhysicalEntities() << " " << vert.getElemTag() << " "
                << (vert.getNodes())[0] << std::endl;
      offset_elements += 1;
   }

   for (size_t i = 0; i < _edges.size(); i++) {
      Edge edge(_edges[i]);
      mesh_file << offset_elements << " " << 1 << " " << 2 //number of tag
                << " " << edge.getPhysicalEntities() << " " << edge.getElemTag() << " "
                << (edge.getNodes())[0] << " " << (edge.getNodes())[1] << std::endl;
      offset_elements += 1;
   }

   for (size_t i = 0; i < _triangles.size(); i++) {
      Triangle tri(_triangles[i]);
      mesh_file << offset_elements << " " << 2 << " " << 2 //number of tag
                << " " << tri.getPhysicalEntities() << " " << tri.getElemTag() << " "
                << (tri.getNodes())[0] << " " << (tri.getNodes())[1] << " " << (tri.getNodes())[2]
                << std::endl;
      offset_elements += 1;
   }
   ...
   for (size_t i = 0; i < _quadrangles.size(); i++) {
      Quadrangle quad(_quadrangles[i]);
      mesh_file << offset_elements << " " << 3 << " " << 2 //number of tag
                << " " << quad.getPhysicalEntities() << " " << quad.getElemTag() << " "
                << (quad.getNodes())[0] << " " << (quad.getNodes())[1] << " "
                << (quad.getNodes())[2] << " " << (quad.getNodes())[3] << std::endl;
      offset_elements += 1;
   }

   for (size_t i = 0; i < _tetrahedra.size(); i++) {
      Tetrahedron tet(_tetrahedra[i]);
      mesh_file << offset_elements << " " << 4 << " " << 2 //number of tag
                << " " << tet.getPhysicalEntities() << " " << tet.getElemTag() << " "
                << (tet.getNodes())[0] << " " << (tet.getNodes())[1] << " " << (tet.getNodes())[2]
                << " " << (tet.getNodes())[3] << std::endl;
      offset_elements += 1;
   }

   for (size_t i = 0; i < _hexahedra.size(); i++) {
      Hexahedron hexa(_hexahedra[i]);
      mesh_file << offset_elements << " " << 5 << " " << 2 //number of tag
                << " " << hexa.getPhysicalEntities() << " " << hexa.getElemTag() << " "
                << (hexa.getNodes())[0] << " " << (hexa.getNodes())[1] << " "
                << (hexa.getNodes())[2] << " " << (hexa.getNodes())[3] << " "
                << (hexa.getNodes())[4] << " " << (hexa.getNodes())[5] << " "
                << (hexa.getNodes())[6] << " " << (hexa.getNodes())[7] << std::endl;
      offset_elements += 1;
   }

   for (size_t i = 0; i < _prisms.size(); i++) {
      Prism pri(_prisms[i]);
      mesh_file << offset_elements << " " << 6 << " " << 2 //number of tag
                << " " << pri.getPhysicalEntities() << " " << pri.getElemTag() << " "
                << (pri.getNodes())[0] << " " << (pri.getNodes())[1] << " " << (pri.getNodes())[2]
                << " " << (pri.getNodes())[3] << " " << (pri.getNodes())[4] << " "
                << (pri.getNodes())[5] << std::endl;
      offset_elements += 1;
   }

   for (size_t i = 0; i < _pyramids.size(); i++) {
      Pyramid py(_pyramids[i]);
      mesh_file << offset_elements << " " << 7 << " " << 2 //number of tag
                << " " << py.getPhysicalEntities() << " " << py.getElemTag() << " "
                << (py.getNodes())[0] << " " << (py.getNodes())[1] << " " << (py.getNodes())[2]
                << " " << (py.getNodes())[3] << " " << (py.getNodes())[4] << std::endl;
      offset_elements += 1;
   }
...
void
Gmesh::writeGmesh(const std::string name_mesh, const size_t format) const
{
   if (format == 1) {
...
void
Gmesh::addNode(const Node& node)
{
   _nodes.push_back(node);
}

void
Gmesh::addVertice(const Vertice& vertice)
{
...
void
Gmesh::addTriangle(const Triangle& triangle)
{
   _triangles.push_back(triangle);
   _number_of_elements += 1;
}
...
void
Gmesh::addQuadrangle(const Quadrangle& quad)
{
   _quadrangles.push_back(quad);
   _number_of_elements += 1;
}
...
void
Gmesh::addHexahedron(const Hexahedron& hexa)
{
   _hexahedra.push_back(hexa);
   _number_of_elements += 1;
}
...
void
Gmesh::addTetrahedron(const Tetrahedron& tetra)
{
   _tetrahedra.push_back(tetra);
   _number_of_elements += 1;
}
...
void
Gmesh::addPrism(const Prism& prism)
{
   _prisms.push_back(prism);
   _number_of_elements += 1;
}
...
void
Gmesh::addPyramid(const Pyramid& pyramid)
{
...
void
Gmesh::convertInDiscontinuousMesh1()
{
...
void
Gmesh::convertInDiscontinuousMesh2()
{
...
void
Gmesh::convertInDiscontinuousMesh3()
{
   const size_t numberofNewNodes =
     4 * _tetrahedra.size() + 5 * _pyramids.size() + 6 * _prisms.size() + 8 * _hexahedra.size();
   std::vector<Node> oldNodes(_nodes);
...
void
Gmesh::convertInDiscontinuousMesh()
{
   if (_dim_topology == 1) {
...
void
Gmesh::computeDeformed(const std::vector<Node>& newNodes)
{
