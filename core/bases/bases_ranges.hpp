...
take(const dynamic_matrix<T>& from, const dof_range& row_range, const dof_range& col_range)
{
   return from.block(row_range.from(), col_range.from(), row_range.size(), col_range.size());
}
