...
class monomial_generator
{
   ...
   {
      bool
      operator()(const monomial_type& a, const monomial_type& b)
      {
         ...
         if (sum_a == sum_b) return val_a < val_b;
   ...
   make_index_table(void)
   {
      ...
      for (size_t i = 0; i < m_monomials.size(); i++) {
         ...
         if (msum > prev_degree) m_degree_indices.at(cur_deg++) = i;
 ...
 public:
   monomial_generator() : m_max_degree(1) { generate_monomials(m_max_degree); }

   monomial_generator(size_t max_degree) : m_max_degree(max_degree)
   {
   ...
   size_t
   degree_position(size_t degree) const
   {
