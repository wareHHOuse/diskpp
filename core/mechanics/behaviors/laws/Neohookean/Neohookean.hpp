...
class Neohookean
{
  private:
    ...
    size_t                                                            m_nb_qp;
    std::vector<Neohookean_cell<mesh_type>> m_list_cell_qp;
    Neohookean_Data<scalar_type>            m_data;

  public:
    Neohookean() : m_nb_qp(0){};

    Neohookean(const mesh_type& msh, const int degree)
    {
        m_nb_qp = 0;
        ...
        for (auto& cl : msh)
        {
            Neohookean_cell<mesh_type> cell_qp(msh, cl, degree);
    ...
    addMaterialData(const scalar_type& lambda,
                    ...
                    const size_t& type)
    {
        m_data = Neohookean_Data<scalar_type>(lambda, mu, type);
    }

    Neohookean_Data<scalar_type>
