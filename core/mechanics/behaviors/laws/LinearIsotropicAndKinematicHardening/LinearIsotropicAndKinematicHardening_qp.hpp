...
class LinearIsotropicAndKinematicHardening_Data
{
 ...
 public:
   LinearIsotropicAndKinematicHardening_Data() :
     m_lambda(1.0), m_mu(1.0), m_H(0), m_K(0), m_sigma_y0(10E60)
   {}
...
class LinearIsotropicAndKinematicHardening_qp
{
 ...
 public:
   LinearIsotropicAndKinematicHardening_qp(const point<scalar_type, DIM>& point,
     ...
     m_point(point),
     m_weight(weight), m_estrain_prev(zero_matrix), m_pstrain_prev(zero_matrix),
     m_p_prev(scalar_type(0)), m_estrain_curr(zero_matrix), m_pstrain_curr(zero_matrix),
   ...
   compute_stress(const data_type& data) const
   {
      ...
      const auto stress =
        2 * data.getMu() * m_estrain_curr + data.getLambda() * m_estrain_curr.trace() * Id;

      return stress.block(0, 0, DIM, DIM);
   }

   std::pair<static_matrix_type, static_tensor<scalar_type, DIM>>
   compute_whole(const static_matrix_type& incr_strain,
                 ...
                 bool                      tangentmodulus = true)
   {
      ...
      const static_matrix_type3D estrain_trial =
        m_estrain_prev + convert3D(incr_strain);                        // elastic strain trial
      ...
      if (m_is_plastic_curr) {
         const scalar_type dem =
           3 * data.getMu() + data.getH() + scalar_type(3.) * data.getK() / scalar_type(2.);
         ...
         if (tangentmodulus) {
            ...
            Cep +=
              4 * mu2 * (delta_p / se_eq - 1.0 / dem) * nxn - 6.0 * mu2 * delta_p / se_eq * Pdev;
         }
      } else {
         // update
