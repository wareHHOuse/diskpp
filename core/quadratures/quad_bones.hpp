...
tetrahedron_quadrature(size_t degree)
{
    ...
    {
#ifdef USE_ARBQ
        const double w = ws[i] / (std::sqrt(8.) / 3.);
#else
        const double w = ws[i];
#endif

        const auto r = pts[3 * i];
        const auto s = pts[3 * i + 1];
        const auto t = pts[3 * i + 2];

        const auto p = point<double, 3>{r, s, t};

#ifdef USE_ARBQ
        const static_vector<double, 3> q = invA * (p.to_vector() - b);
        const auto qq = point<double, 3>{q(0), q(1), q(2)};
...
edge_quadrature(size_t doe)
{
    ...
    if (num_nodes == 1)
    {
        const auto qp = std::make_pair(point<T, 1>({0.5}), 1.0);
        ret.push_back(qp);
        return ret;
    }
    ...
    const dynamic_vector<T> nodes = solver.eigenvalues();

    assert(weights.size() == nodes.size());

    for (size_t i = 0; i < nodes.size(); i++)
    {
        const auto qp = std::make_pair(point<T, 1>({(nodes(i) + 1.) / 2.}), weights(i));
        ret.push_back(qp);
    }
