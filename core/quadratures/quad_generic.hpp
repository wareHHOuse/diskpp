...
class quadrature<generic_mesh<T, 2>, typename generic_mesh<T, 2>::cell>
{
 ...
 private:
   ...
   std::vector<quadpoint_type>
   integrate_triangle(const mesh_type& msh, const cell_type& cl, const PtA& pts) const
   {
      std::vector<quadpoint_type> ret;
      ...
      std::transform(m_quadrature_data.begin(), m_quadrature_data.end(), retbegin, tr);
   ...
   template<typename PtA>
   std::vector<quadpoint_type>
   integrate_quad(const mesh_type& msh, const cell_type& cl, const PtA& pts) const
   {
      std::vector<quadpoint_type> ret;
...
//#define OPTIMAL_TRIANGLE_NUMBER
   ...
   template<typename PtA>
   std::vector<quadpoint_type>
   integrate_other(const mesh_type& msh, const cell_type& cl, const PtA& pts) const
   {
      std::vector<quadpoint_type> ret;
 ...
 public:
   ...
   integrate(const mesh_type& msh, const cell_type& cl) const
   {
      ...
      switch (pts.size()) {
         case 3: return integrate_triangle(msh, cl, pts);

         case 4: return integrate_quad(msh, cl, pts);

         default: return integrate_other(msh, cl, pts);
      }
