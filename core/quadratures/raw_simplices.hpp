...
auto points(const raw_simplex<T, N> &rs)
{
    return rs.points();
}

template <typename T>
auto measure(const raw_simplex<T, 1> &rs)
{
...
template <typename T>
auto measure(const raw_simplex<T, 2> &rs)
{
    const auto pts = rs.points();
    assert(pts.size() == 3);
...
template <typename T>
auto measure(const raw_simplex<point<T, 3>, 3> &rs)
{
    const auto pts = rs.points();
    assert(pts.size() == 4);
...
template <typename T, size_t N>
T barycenter(const raw_simplex<T, N> &rs)
{
    const auto pts = rs.points();
...
split_in_raw_triangles(const Mesh &msh, const typename Mesh::face &fc)
{
    ...
    else
    {
        ...
        {
            const auto rs = raw_simplex_type({bar, pts[i], pts[(i + 1) % pts.size()]});
