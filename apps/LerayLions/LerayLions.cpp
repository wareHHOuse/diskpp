...
void
run_leraylions_solver(const Mesh<T, 1, Storage>& msh, ParamRun<T>& rp)
{
   typedef T result_type;

   auto load = [rp](const point<T, 1>& pt) -> result_type {
      size_t p = rp.m_leray_param;

      const T x5 = std::pow(pt.x(), 5);
      const T x6 = pt.x() * x5;

      return -14 * x5 * std::pow(x6 * (8 * pt.x() - 7), p - 1) * (4 * pt.x() - 3);
   };

   auto solution = [](const point<T, 1>& pt) -> result_type {
      return std::pow(pt.x(), 7) * (pt.x() - 1);
   };

   leraylions_solver<Mesh, T, 1, Storage> nl(msh, rp);
   ...
   while ((ch = getopt(argc, argv, "n:r:")) != -1) {
      switch (ch) {
         case 'n':
            ...
            if (elems_1d < 0) {
               std::cout << " n has to pe positive. Falling back to 8." << std::endl;
               elems_1d = 8;
            }
            break;
