...
class leraylions_solver
{
   ...
   init()
   {
      ...
      if (m_verbose) {
         ...
         std::cout << "** Numbers of dofs: " << m_msh.faces_size() * num_face_dofs + total_lagr
                   << std::endl;
         std::cout << "** including " << total_lagr << " Lagrange multipliers" << std::endl;
      }
      ...
      for (size_t i = 0; i < m_msh.cells_size(); i++) {
         m_solution_data[i].setConstant(0.0001);
         m_solution_cells[i].setConstant(0.0001);
      }

      for (size_t i = 0; i < m_msh.faces_size(); i++) {
         m_solution_faces[i].setConstant(0.0001);
      }

      for (size_t i = 0; i < m_msh.boundary_faces_size(); i++) {
         m_solution_lagr[i].setConstant(0.0001);
      }
   }
 ...
 public:
   ...
   //       visu::NodeData nodedata(1, 0.0, "sol", data, subdata); // create and init a nodedata
   //       view
   ...
   //               1, std::abs(sol - true_depl)); // save the solution at gauss point
   ...
   //         1, 0.0, "error_sol", data, subdata); // create and init a nodedata view
