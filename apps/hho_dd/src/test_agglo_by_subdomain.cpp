...
int main(void)
{
    ...
    partition_unit_square_mesh(finemsh, 2);
    ...
    size_t ci = 0;
    for (const auto& [ccl, ffcs] : cc2ff.first) {
        std::ofstream ofs("cell_"+std::to_string(ci)+".m");
        for (const auto& ffc : ffcs) {
            auto pts = points(finemsh, ffc);
            ofs << "line(["
                << pts[0].x() << ", " << pts[1].x() << "], ["
                << pts[0].y() << ", " << pts[1].y() << "], ";
            if ( cc2ff.second[ offset(finemsh, ffc) ] == 1 )
                ofs << "'Color', 'black'";
            else
                ofs << "'Color', 'red'";
