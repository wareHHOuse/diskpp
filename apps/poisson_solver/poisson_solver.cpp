...
hho_solver(sol::state& lua, const Mesh& msh)
{
   ...
   const auto bqd = bqdata_type(degree, degree);

   auto gradrec   = gradrec_type(bqd);
   auto stab      = stab_type(bqd);
   auto statcond  = statcond_type(bqd);
   auto assembler = assembler_type(msh, bqd);

   auto bcf = [](const typename mesh_type::point_type& pt) -> auto { return 0; };
   ...
   for (auto& cl : msh) {
      ...
      assembler.assemble(msh, cl, scnp, bcf);

      if (elem_i % 10000 == 0)
         std::cout << "\33[2K\rAssembly " << double(100 * elem_i) / msh.cells_size() << "%"
                   << std::flush;
   }
   ...
   const size_t systsz = system_matrix.rows();
   const size_t nnz    = system_matrix.nonZeros();
   ...
   const auto num_rec_dofs = bqd.cell_basis.computed_size();
   ...
   for (auto& cl : msh) {
      ...
      const auto                  num_local_face_dofs = num_face_dofs * num_faces;
      dynamic_vector<scalar_type> local_face_dofs =
        dynamic_vector<scalar_type>::Zero(num_local_face_dofs);

      size_t face_i = 0;
      for (auto& fc : fcs) {
         const size_t face_ofs        = msh.lookup(fc);
         const auto   local_face_ofs  = face_i * num_face_dofs;
         const auto   global_face_ofs = face_ofs * num_face_dofs;
      ...
      const dynamic_vector<scalar_type> local_dofs =
        statcond.recover(msh, cl, loc, cell_rhs, local_face_dofs);

      const dynamic_vector<scalar_type> R = gradrec.oper * local_dofs;
      ...
      const dynamic_vector<scalar_type> vT  = local_dofs.segment(0, num_cell_dofs);
      const auto                        phi = bqd.cell_basis.eval_functions(msh, cl, bar);
      vals.at(elem_i)                       = vT.dot(phi);

      const auto phi1    = bqd.cell_basis.eval_functions(msh, cl, bar, 1, degree + 1);
      ...
      if (expected_solution.valid()) {
         ...
         dynamic_vector<scalar_type> cell_rhs = dynamic_vector<scalar_type>::Zero(num_cell_dofs);
   ...
   if (expected_solution.valid()) std::cout << "L2-norm error: " << sqrt(error) << std::endl;
   ...
   if (hdf5_output_filename.valid()) {
      ...
      hid_t dataset_id = H5Dcreate(
        group_id, "solution", H5T_IEEE_F64LE, dataspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
      ...
      H5Dwrite(dataset_id,
               ...
               H5S_ALL,
               H5P_DEFAULT,
               reconstructed_solution.data());

      hsize_t dims              = 1;
      hid_t   attr_dataspace_id = H5Screate_simple(1, &dims, NULL);

      hid_t attribute_id = H5Acreate2(
        dataset_id, "degree", H5T_STD_I32BE, attr_dataspace_id, H5P_DEFAULT, H5P_DEFAULT);
      ...
      attribute_id =
        H5Acreate2(dataset_id, "elems", H5T_STD_I32BE, attr_dataspace_id, H5P_DEFAULT, H5P_DEFAULT);
...
cfem_eigenvalue_solver(sol::state& lua, const disk::simplicial_mesh<T, 2>& msh)
{
   ...
   size_t system_size = std::count_if(
     dirichlet_nodes.begin(), dirichlet_nodes.end(), [](bool d) -> bool { return !d; });
   expand_map.resize(system_size);

   auto nnum = 0;
   for (size_t i = 0; i < msh.points_size(); i++) {
      if (dirichlet_nodes.at(i)) continue;
   ...
   for (auto& cl : msh) {
      ...
      for (size_t i = 0; i < K.rows(); i++) {
         if (dirichlet_nodes.at(ptids[i])) continue;

         for (size_t j = 0; j < K.cols(); j++) {
            if (dirichlet_nodes.at(ptids[j])) continue;

            gtK.push_back(
              triplet_type(compress_map.at(ptids[i]), compress_map.at(ptids[j]), K(i, j)));

            gtM.push_back(
              triplet_type(compress_map.at(ptids[i]), compress_map.at(ptids[j]), M(i, j)));
         }
      }
   }
...
cfem_solver(sol::state& lua, const disk::simplicial_mesh<T, 2>& msh)
{
   ...
   for (auto& cl : msh) {
      ...
      for (size_t i = 0; i < A.rows(); i++) {
         ...
         for (size_t j = 0; j < A.cols(); j++) {
            ...
            triplets.push_back(
              triplet_type(compress_map.at(ptids[i]), compress_map.at(ptids[j]), A(i, j)));
         }
   ...
   if (hdf5_output_filename.valid()) {
      ...
      status = H5Dwrite(dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, e_gx.data());
...
eigval_solver(sol::state& lua, const Mesh& msh)
{
   ...
   const size_t num_global_cell_dofs = msh.cells_size() * num_cell_dofs;
   const size_t num_global_face_dofs = msh.internal_faces_size() * num_face_dofs;
   const size_t num_global_dofs      = num_global_cell_dofs + num_global_face_dofs;

   /* Strong imposition of dirichlet bc */
   std::vector<size_t> compress_map, expand_map;
   compress_map.resize(msh.faces_size());
   const size_t num_non_dirichlet_faces = msh.internal_faces_size();
   const size_t system_size             = num_non_dirichlet_faces * num_face_dofs;
   expand_map.resize(system_size);
   ...
   for (auto& cl : msh) {
      ...
      const auto K = gradrec.data + stab_weight * stab.data;
      const auto M = mass.data;
      ...
      for (size_t i = 0; i < num_faces; i++) {
         ...
         if (msh.is_boundary(fc)) {
            ...
            const auto face_pos = compress_map.at(face_id);
            // std::cout << face_id << " -> " << face_pos << std::endl;
            for (size_t j = 0; j < num_face_dofs; j++)
               offset_table.at(num_cell_dofs + i * num_face_dofs + j) =
                 num_cell_dofs * msh.cells_size() + num_face_dofs * face_pos + j;
         }
      }

      /* K has cell and face parts */
      for (size_t i = 0; i < K.rows(); i++) {
         const size_t i_ofs = offset_table.at(i);
         if (i_ofs == 0xdeadbeef) continue;

         for (size_t j = 0; j < K.cols(); j++) {
            const size_t j_ofs = offset_table.at(j);
            if (j_ofs == 0xdeadbeef) continue;
   ...
   if (visit_output_filename.valid()) {
      ...
      for (size_t i = 0; i < mode_x_max; i++) {
         for (size_t j = 0; j < mode_y_max; j++) {
            ...
            std::cout << i << " " << j << " "
                      << M_PI * (i + 1) * M_PI * (i + 1) + M_PI * (j + 1) * M_PI * (j + 1)
                      << std::endl;
