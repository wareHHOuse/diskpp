...
template <typename T>
auto to_angle(const disk::point<T, 2> &p, const disk::point<T, 2> &o) {
  return atan2(p.y() - o.y(), p.x() - o.x());
}

template <typename Mesh, typename Points>
auto sort_by_polar_angle(const Mesh &msh, const typename Mesh::cell &cl,
                         const Points &pts) {
  typedef disk::point<typename Mesh::coordinate_type, 2> point_type;
  // Warning this may work only on convex polygons
 ...
 */
template <typename T>
auto isLeft(const disk::point<T, 2> &P0, const disk::point<T, 2> &P1,
            const disk::point<T, 2> &P2) {
...
bool wn_PnPoly(const Mesh &msh, const typename Mesh::cell &cl,
               const disk::point<typename Mesh::coordinate_type, 2> &P) {
  auto vts = points(msh, cl);

  typedef typename Mesh::coordinate_type scalar_type;

  std::vector<disk::point<scalar_type, 2>> svts(vts.size() + 1);
...
void plot_over_line(
    const Mesh &msh,
    const std::pair<disk::point<typename Mesh::coordinate_type, 2>,
                    disk::point<typename Mesh::coordinate_type, 2>> &e,
    const Matrix<typename Mesh::coordinate_type, Dynamic, 1> &sol,
    const typename disk::hho_degree_info &hdi, const std::string &filename) {
...
template <typename Mesh>
void compute_discontinuous_velocity(
    const Mesh &msh,
    const disk::dynamic_vector<typename Mesh::coordinate_type> &sol,
    const typename disk::hho_degree_info &hdi, const std::string &filename) {
  typedef Mesh mesh_type;
  typedef typename Mesh::coordinate_type scalar_type;
  typedef disk::static_vector<scalar_type, Mesh::dimension> vector_dim_type;
  auto dim = Mesh::dimension;
  const size_t cell_degree = hdi.cell_degree();
  const size_t cbs = disk::vector_basis_size(cell_degree, dim, dim);
  ...
  for (size_t i_node = 0; i_node < value.size(); i_node++) {
    const vector_dim_type depl_avr =
...
void post_processing(
    const Mesh &msh, const Assembler &assembler,
...

{
  ...
  for (auto cl : msh) {
    ...
    // this is only for k = 0 (for k >0, it is ok for plotting  but
    // not for div u )
    ...
    Matrix<T, dim, 1> ueval = disk::eval(cell_vel, phi);

#if 0
    // Stress
    Matrix<T, Dynamic, dim> c_dphi;
    if (use_sym_grad) {
      auto cb = disk::make_sym_matrix_monomial_basis(
          msh, cl, di.reconstruction_degree());
    ...
    Matrix<T, Dynamic, 1> Gu = gr.first * svel;
...
template <typename Mesh>
auto run_stokes(const Mesh &msh, size_t degree, bool use_sym_grad = true) {
  typedef Mesh mesh_type;
