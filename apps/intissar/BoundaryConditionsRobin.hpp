...
class BoundaryConditionsScalar2
{
 public:
 ...
 private:
    ...
    std::vector<std::tuple<bool, size_t, size_t, size_t>> m_faces_is_dirichlet, m_faces_is_neumann, m_faces_is_robin;

    size_t m_dirichlet_faces, m_neumann_faces, m_robin_faces;
 ...
 public:
    ...
    BoundaryConditionsScalar2(const mesh_type& msh) :
        m_msh(msh), m_dirichlet_faces(0), m_neumann_faces(0), m_robin_faces(0), m_factor(1)
    {
   ...
   addDirichletEverywhere(const Function& bcf)
   {
     ...
     template<typename Function>
    void
    addRobinBC (const size_t& btype, const size_t& b_id,  const Function& g)
    {

        const size_t g_id = m_robin_func.size();
        m_robin_func.push_back(g);
        for (auto itor = m_msh.boundary_faces_begin(); itor != m_msh.boundary_faces_end(); itor++)
        {
            const auto bfc = *itor;
            ...
            {
                m_faces_is_robin.at(face_id) = std::make_tuple(true, btype, b_id, g_id);
                m_robin_faces++;
            }
        }
    ...
    robin_boundary_func(const size_t face_i) const
    {
        ...
        {
            throw std::logic_error(
           "You want the Robin function of face which is not a Robin face");
        }
