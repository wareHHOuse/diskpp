...
int main(int argc, char **argv)
{
    ...
    typedef typename disk::mesh_v2::mesh_traits<mesh_type>::node_type   nodet;
    ...
    typedef typename disk::mesh_v2::mesh_traits<mesh_type>::edge_type   edget;
    be->edges.push_back( edget({0,1}) );
    be->edges.push_back( edget({1,3}) );
    be->edges.push_back( edget({0,3}) );
    be->edges.push_back( edget({1,2}) );
    be->edges.push_back( edget({2,3}, 1) );
    //be->edges.push_back({3,1});

    std::sort(be->edges.begin(), be->edges.end());

    typedef typename disk::mesh_v2::mesh_traits<mesh_type>::surface_type surft;

    be->surfaces.push_back( surft({0,1,3}, 8) );
    be->surfaces.push_back( surft({1,2,3}, 9) );


    for (auto& cl : msh)
    {
        std::cout << cl << std::endl;
        std::cout << measure(msh, cl) << std::endl;
    ...
    {
        std::cout << *itor << std::endl;
        std::cout << measure(msh, *itor) << std::endl;
    }
#endif
}
