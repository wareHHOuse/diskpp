...
template <typename Mesh>
auto
make_lhs_t2 (const Mesh& msh, const typename Mesh::cell_type& cl, const hho_degree_info& hdi,
              ...
              const size_t& gamma_N)
{
    ...
    auto fcs=faces(msh, cl);
    auto rbs = scalar_basis_size(recdeg, DIM);
    auto fbs = scalar_basis_size(facdeg, DIM);
    Matrix<T, 1, Dynamic> gradalpha = Matrix<T, 1, Dynamic>::Zero(2, fbs);
    Matrix<T, 1, Dynamic> gradv = Matrix<T, 1, Dynamic>::Zero(2, fbs);
    Matrix<T, 1, Dynamic> alpha = Matrix<T, 1, Dynamic>::Zero(fbs, 1);
    Matrix<T, 1, Dynamic> v = Matrix<T, 1, Dynamic>::Zero(fbs, 1);
    for (auto& fc:fcs)
    {
        auto eid = find_element_id(msh.faces_begin(), msh.faces_end(), fc);
        if (!eid.first) throw std::invalid_argument("This is a bug: face not found");
        const auto face_id=eid.second;
        if (is_contact_face(face_id))
        {
            auto qps = integrate (msh, fc, 2*facdeg);
            auto normale = normal(msh, cl, fc);
            for (auto& qp : qps)
            {
                auto dphi = cb.eval_gradients(qp.point());
                gradalpha += qp.weight() * dphi * dphi.transpose();
                gradv = dphi * dphi.transpose();
            }
             auto qps_1 = integrate(msh, fc, facdeg);
             for (auto& qp : qps_1)
             {
                auto phi = fb.eval_functions(qp.point());
                alpha += qp.weight() * phi * phi.transpose();
                v = phi * phi.transpose();
             }
             auto sigmaalpha = normale.transpose() * gradalpha * reconstruction ;
             auto sigmav = normale.transpose() * gradv * reconstruction ;
             auto integ1 = sigmaalpha * sigmav.transpose();
             auto integ2 = -gamma_N * sigmaalpha * v;
             auto integ3 = -gamma_N * sigmav * alpha;
             auto integ4 = gamma_N * gamma_N *  alpha.transpose() * v;
    ...
    t2=(-1/gamma_N) * (integ1 + integ2 + integ3 + integ4);
    return t2;
}
#endif
            ...
            const Matrix<typename Mesh::coordinate_type, Dynamic, 1>& uloc)
{
    ...
    Matrix<T, Dynamic, 1> rhs = Matrix<T, Dynamic, 1>::Zero(cbs + fcs.size()* fbs, 1);
    ...
    for (auto& fc:fcs)
    {
        ...
        if (bnd.is_contact_face(face_id))
        {
            ...
            for (auto& qp : qps)
            {
                Matrix<T, Dynamic,  Mesh::dimension> c_dphi_tmp = cb.eval_gradients(qp.point());
                Matrix<T, Dynamic,  Mesh::dimension> c_dphi = c_dphi_tmp.block(1, 0, rbs-1, DIM);
                Matrix<T, Dynamic, 1> f_phi = fb.eval_functions(qp.point());
                Matrix<T, Dynamic, 1> uF    = uloc.block(face_ofs, 0, fbs, 1);
                T gamma_u =  gamma_N * f_phi.dot(uF);
                T sigma_n = (c_dphi * n ).dot( rec * uloc);
                T sigman_gamma_u = sigma_n - gamma_u;
                Matrix<T, Dynamic, 1> sigman_gamma_v =  (c_dphi * n).transpose() * rec;
                sigman_gamma_v.block(face_ofs, 0, fbs, 1) = - gamma_N * f_phi;

                rhs = qp.weight() * std::min( sigman_gamma_u, 0.) * sigman_gamma_v;
            }
        }
        fc_count++;
    }
                          ...
                          const hho_degree_info& di)
{
    ...
    for (auto& qp : qps)
    {
        ...
        stiff += qp.weight() * priv::outer_product(phi,phi);
...
project_function(const Mesh& msh, const typename Mesh::cell_type& cl,
                 const hho_degree_info& hdi,
                 const vector_rhs_function<Mesh>& f, size_t di )
{
    using T = typename Mesh::coordinate_type;
...
class diffusion_condensed_assembler2
{
    ...
    size_t      num_all_faces,  num_other_faces,system_size;
    size_t      num_dirichlet_faces, num_neumann_faces, num_robin_faces;
        ...
        if (bnd.nb_faces_neumann() > 0)
        {
            for (auto itor = msh.boundary_faces_begin(); itor != msh.boundary_faces_end(); itor++)
            {
                ...
                if (bnd.is_neumann_face(face_id))
                {
                    ...
                    {
                        switch (bnd.robin_boundary_type(face_id))
                        {
                            case disk::mechanics::ROBIN:
                                throw std::invalid_argument("You tried ti impose both Robin annd Neumann conditions on the same face");
                                break;
                            default:
                                throw std::logic_error("Unknown Robin Conditions");
                            break;
                        }
                    }
                    else
