...
public:
    ...
    const disk::mechanics::scalar_boundary_conditions<Mesh>& bnd,
    ...
    *-------------------------------------------------------------------------*/
    ...
    switch (ap.solver)
    {
        ...
        case EVAL_IN_CELLS_AS_FACES:
            ns.solve_cell_as_faces(msh, force, bnd);
            break;
        case EVAL_WITH_PARAMETER:
            ns.solve_with_parameter(msh, force, bnd, parameter);
            break;
        default:
...
int main(int argc, char **argv)
{
    ...
    while ( (ch = getopt(argc, argv, "k:g:npzfcoe:")) != -1 )
    {
        switch(ch)
        {
            case 'k':
            ...
            case 'f':
                ...
                break;
            case 'c':
                std::cout << "Not solving with cells yet" << std::endl;;
                break;
            case 'o':
