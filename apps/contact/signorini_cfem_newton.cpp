...
public:
    ...
    cfem_newton_solver(const disk::simplicial_mesh<T, 2>& msh,
        ...
        const algorithm_parameters<T>& ap)
    {
        ...
        for(size_t iter= 0; iter < maxiter ; iter++)
        {
            ...
            for (auto& cl : msh)
            {
                ...
                matrix_type Aheaviside = matrix_type::Zero();
                ...
                algorithm_parameters<T>& ap)
{
    ...
    disk::mechanics::BoundaryConditionsScalar<disk::simplicial_mesh<T, 2>> bnd(msh);
    ...
    cfem_newton_solver<T> wfem(msh, bnd); //, ap, rhs_fun);
    wfem.solve(msh, force, bnd, ap);
    wfem.postprocess(msh);
