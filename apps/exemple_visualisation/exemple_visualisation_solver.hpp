   ...
   plot_l2error_at_gausspoint(const std::string& filename, const AnalyticalSolution& as)
   {
      ...
      for (auto& cl : m_msh)
      {
         ...
         for (auto& qp : qps)
         {
            ...
            nb_node += 1;
            const visu::Node snode = visu::convertPoint(qp.point(), nb_node); //create a node at gauss point
            const std::vector<double> value = {std::fabs(pot - true_pot)}; // save the solution at gauss point
