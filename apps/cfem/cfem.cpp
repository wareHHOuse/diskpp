...
cfem_solver(const disk::simplicial_mesh<T, 2>& msh)
{
   ...
   const size_t system_size = std::count_if(
   ...
   for (auto& cl : msh) {
      ...
      const auto c = std::cos(M_PI * bar.x() / 0.02);
      const auto s = std::sin(M_PI * bar.y() / 0.02);
      const auto eps =
        1 + 100 * c * c * s * s; // + std::exp( (bar.x()*bar.x() + bar.y()*bar.y())/2. );
      kappa(0, 0) = eps;
      kappa(1, 1) = eps;

      const auto A = disk::cfem::stiffness_matrix(msh, cl, kappa);
      const auto b = disk::cfem::make_rhs(msh, cl, f);

      const auto ptids = cl.point_ids();
   ...
   const size_t systsz = gA.rows();
   const size_t nnz    = gA.nonZeros();
   ...
   for (auto& cl : msh) {
      const auto bar   = barycenter(msh, cl);
      const auto phi   = disk::cfem::eval_basis(msh, cl, bar);
      const auto ptids = cl.point_ids();
