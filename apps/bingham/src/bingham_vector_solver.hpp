...
public:
                        ...
                        const vector_type& velocity)
    {
        ...
        matrix_type  theta_eval = disk::eval(theta, s_phi);
    ...
    post_processing(const mesh_type& msh, Assembler& assembler, const size_t iter)
    {
        ...
        for(auto cl : msh)
        {
            ...
            vector_type ueval = disk::eval(cell_vel, phi);
            ...
            vector_type   theta  = stress  +  2. * vp.alpha * Gu;
            matrix_type   theta_eval = disk::eval(theta, s_phi);
            matrix_type   sigma_eval = disk::eval(stress, s_phi);
            matrix_type   Gu_eval = disk::eval(Gu, s_phi);
