                   ...
                   const typename Mesh<T,2,Storage>::coordinate_type& omega)
{
    ...
    maxwell_assembler_condensed<mesh_type> assm(msh, chdi);
#else
...
void autotest_convergence(size_t order_min, size_t order_max)
{
    ...
    for (size_t i = 1; i <= 5; i++)
    {
        ...
        for (size_t order = order_min; order <= order_max; order++)
        {
            ...
            auto ci = vector_wave_solver(msh, order, sp[order], M_PI, false);
...
int main(int argc, char **argv)
{
    ...
    if (std::regex_match(mesh_filename, std::regex(".*\\.mesh$") ))
    {
        ...
        if (solve_eigvals)
            ...
            vector_wave_solver(msh, degree, stab_param, omega, use_ho_stab);
