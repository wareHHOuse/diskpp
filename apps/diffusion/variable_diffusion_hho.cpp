...
run_hho_variable_diffusion_solver(const Mesh& msh, const size_t degree)
{
    ...
    for (auto& cl : msh)
    {
        ...
        const auto sc = diffusion_static_condensation_compute(msh, cl, hdi, gr.second, rhs);
    ...
    for (auto& cl : msh)
    {
        ...
        vector_type sol = diffusion_static_condensation_recover(msh, cl, hdi, gr.second, rhs, locsol);
