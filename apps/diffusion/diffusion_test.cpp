...
template<typename MeshType, typename LoaderType>
bool
verify_convergence1D(const std::vector<size_t>& n_elems, size_t mindeg, size_t maxdeg)
{
   typedef typename MeshType::scalar_type scalar_type;

   auto f = [](const point<scalar_type, 1>& p) -> auto { return M_PI * M_PI * sin(p.x() * M_PI); };

   auto sf = [](const point<scalar_type, 1>& p) -> auto { return sin(p.x() * M_PI); };
...
template<typename MeshType, typename LoaderType>
bool
verify_convergence(const std::vector<std::string>& paths, size_t mindeg, size_t maxdeg)
{
   typedef typename MeshType::scalar_type scalar_type;
...
test_1D()
{
   ...
   typedef disk::generic_mesh<double, 1>        MT;
   typedef disk::uniform_mesh_loader<double, 1> LT;

   verify_convergence1D<MT, LT>(n_elems, 0, 3);
}

void
test_triangles_specialized(test_type tt)
{
   size_t runs = 2;
   ...
   switch (tt) {
      case TEST_MEASURE_TIMES: test_mesh_format<MT, LT>(paths, runs, 0, 3, "triangle_spec"); break;

      case TEST_VERIFY_CONVERGENCE: verify_convergence<MT, LT>(paths, 0, 3); break;

      default: std::cout << "[ Unavailable Test ]" << std::endl; return;
   }
}

void
test_triangles_generic(test_type tt)
{
   size_t runs = 2;
   ...
   switch (tt) {
      case TEST_MEASURE_TIMES: test_mesh_format<MT, LT>(paths, runs, 0, 3, "triangle_gen"); break;
...
void
test_hexagons_generic(test_type tt)
{
   size_t runs = 2;
   ...
   switch (tt) {
      case TEST_MEASURE_TIMES: test_mesh_format<MT, LT>(paths, runs, 0, 3, "hexagons_gen"); break;
...
void
test_hextri_generic(test_type tt)
{
   size_t runs = 2;
   ...
   switch (tt) {
      case TEST_MEASURE_TIMES: test_mesh_format<MT, LT>(paths, runs, 0, 3, "hextri_gen"); break;
...
test_kershaw_2d(test_type tt)
{
   ...
   switch (tt) {
      case TEST_MEASURE_TIMES: test_mesh_format<MT, LT>(paths, runs, 0, 3, "kershaw_2d"); break;
...
void
test_hexahedra_specialized(test_type tt)
{
   size_t runs = 2;
   ...
   switch (tt) {
      case TEST_MEASURE_TIMES: test_mesh_format<MT, LT>(paths, runs, 0, 3, "hexahedra_spec"); break;
...
void
test_hexahedra_generic(test_type tt)
{
   size_t runs = 2;
   ...
   switch (tt) {
      case TEST_MEASURE_TIMES: test_mesh_format<MT, LT>(paths, runs, 0, 3, "hexahedra_gen"); break;
...
void
test_tetrahedra_specialized(test_type tt)
{
   size_t runs = 2;
...
void
test_tetrahedra_generic(test_type tt)
{
   size_t runs = 2;
   ...
   switch (tt) {
      case TEST_MEASURE_TIMES: test_mesh_format<MT, LT>(paths, runs, 0, 3, "tetrahedra_gen"); break;
...
void
test_polyhedra_generic(test_type tt)
{
   size_t runs = 2;
   ...
   switch (tt) {
      case TEST_MEASURE_TIMES: test_mesh_format<MT, LT>(paths, runs, 0, 3, "polyhedra"); break;
...
main(int argc, char** argv)
{
   ...
   while ((ch = getopt(argc, argv, "tc")) != -1) {
      switch (ch) {
         case 't': tt = TEST_MEASURE_TIMES; break;

         case 'c': tt = TEST_VERIFY_CONVERGENCE; break;
   ...
   std::cout << bold << underline << "1D test:" << reset << std::endl;
   test_1D();
